...
class SettingUI:
    ...
    def __init__(self,scr,keys_map):
        ...
        self.back=Button("Back (Esc)",self.font,(SCREEN_WIDTH//2,SCREEN_HEIGHT-50))
    def run(self):
        while True:
            self.clock.tick(FPS)
            for e in pygame.event.get():
                ...
                if e.type==pygame.KEYDOWN and not self.wait and e.key==pygame.K_ESCAPE:
                    cfg.MUSIC_ON, cfg.SFX_ON = self.music_on, self.sfx_on; return
                if e.type==pygame.MOUSEBUTTONDOWN and e.button==1:
                    mx,my=e.pos
                    # 重綁按鍵
                ...
                if e.type==pygame.KEYDOWN and self.wait:
                    self.keys[self.wait]=e.key; self.wait=None
            ...
            mp=pygame.mouse.get_pos(); self.back.draw(self.s,self.back.rect.collidepoint(mp))
            pygame.display.flip()
